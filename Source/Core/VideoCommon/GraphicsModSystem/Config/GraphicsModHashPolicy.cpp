...
{
  return static_cast<std::underlying_type_t<HashAttributes>>(all_attributes & attribute) ==
         0;
}
...
HashAttributes HashAttributesFromString(const std::string& str)
{
  ...
  HashAttributes attributes = HashAttributes::None;
  ...
  if (attributes == HashAttributes::None)
    return GetDefaultHashAttributes();
  return attributes;
}
