  ...
  public:
    ...
    bool Compile() override
    {
      ...
      if (!output_targets.empty())
      {
        ...
        {
          const auto& output_target = output_targets[i];
          const auto& render_target = m_custom_material.shader.material->render_targets[i];
          const u32 texel_size = AbstractTexture::GetTexelSizeForFormat(render_target.format);
