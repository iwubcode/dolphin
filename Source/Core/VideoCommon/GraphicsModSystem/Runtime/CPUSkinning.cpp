                                     ...
                                     std::vector<Eigen::Affine3f>& out_bone_matrices)
{
  ...
  for (auto const& [bone_id, local_bone_group] : chunk_rig_data.bone_groups)
  {
    ...
    // 1. First Pass: Weighted Centroids
    std::vector<Eigen::Vector3f> p_rest_list(N), p_curr_list(N);
    for (size_t i = 0; i < N; ++i)
    {
      p_rest_list[i] = replacement_rig.welded_positions[local_bone_group.welded_indices[i]];

      const u8* read_ptr =
          native_draw_data.vertex_data +
          local_bone_group.original_indices[i] * native_draw_data.vertex_format->GetVertexStride();
      const auto native_position = ReadVec3(
          read_ptr, native_draw_data.vertex_format->GetVertexDeclaration().position.offset);
      const Eigen::Vector3f native_e_pos =
          Eigen::Vector3f(native_position.x, native_position.y, native_position.z);
      p_curr_list[i] =
          (native_e_pos - replacement_rig.welded_rig_centroid) / replacement_rig.welded_rig_scale;

      float w = local_bone_group.weights[i];
      c_rest += p_rest_list[i] * w;
      c_curr += p_curr_list[i] * w;
      total_weight += w;
    }
    ...
    // 2. Second Pass: Weighted Covariance H = Rest * Current^T
    Eigen::Matrix3f H = Eigen::Matrix3f::Zero();
    for (size_t i = 0; i < N; ++i)
    {
      H += local_bone_group.weights[i] * (p_curr_list[i] - c_curr) *
           (p_rest_list[i] - c_rest).transpose();
    }

    // 3. SVD and Rotation Calculation
    Eigen::JacobiSVD<Eigen::Matrix3f> svd(H, Eigen::ComputeFullU | Eigen::ComputeFullV);
    Eigen::Matrix3f U = svd.matrixU();
    Eigen::Matrix3f V = svd.matrixV();

    // The optimal rotation is R = U * V^T
    // Note: Swapping U/V here is critical depending on H calculation order
    ...
    // 4. Final Translation and Affine Build
    Eigen::Affine3f final_T = Eigen::Affine3f::Identity();
    final_T.linear() = R;
    final_T.translation() = c_curr - R * c_rest;

    out_bone_matrices[bone_id] = final_T;
  }
}*/
                      ...
                      VideoCommon::SkeletonInstance* skeleton, std::vector<u8>* out)
{
  ...
  // If local_bone_solutions found a valid transform for Bone 5,
  // we update our 'Active_Skeleton' with it.

  // ERROR_LOG_FMT(VIDEO, "Draw call '{}' has bones:", std::to_underlying(draw_call));

  for (std::size_t bone_id = 0; bone_id < local_bone_solutions.size(); ++bone_id)
  {
    /*const auto bone_group_iter = it->second.bone_groups.find(static_cast<int>(bone_id));
    if (bone_group_iter == it->second.bone_groups.end())
      continue;*/
    ...
    if (local_bone_weights[bone_id] > skeleton->best_confidence[bone_id])
    {
      ...
      {
        // ERROR_LOG_FMT(VIDEO, "Bone: {}, total weight: {}", bone_id, local_bone_weights[bone_id]);
  ...
  // Even though we don't have the "Whole Body" yet, we can skin
  // this specific chunk using the current 'Active_Skeleton'
  // and send it to the GPU.
  ApplySkinningLBS(replacement_chunk, replacement_rig, skeleton->bone_matrices, out);
}
}  // namespace GraphicsModSystem
