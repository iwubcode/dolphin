...
void WriteDefines(ShaderCode* out, const std::vector<std::string>& texture_code_names,
                  u32 texture_unit)
{
  for (int i = 0; i < texture_code_names.size(); i++)
  {
...
std::unique_ptr<CustomPipelineAction> CustomPipelineAction::Create(const picojson::value& json_data,
                                                                   std::string_view path)
{
  ...
  const auto& assets_json = json_data.get("assets");
  if (assets_json.is<picojson::object>())
  {
    for (const auto& [asset_id, asset_map] : assets_json.get<picojson::object>())
    {
      VideoCommon::DirectFilesystemAssetLibrary::AssetMap map;
      if (!asset_map.is<picojson::object>())
      {
        ERROR_LOG_FMT(VIDEO,
                      "Failed to load custom pipeline action, asset id '{}' "
                      "is not an object!",
                      asset_id);
        return nullptr;
      }
      for (const auto& [key, value] : asset_map.get<picojson::object>())
      {
        if (!value.is<std::string>())
        {
          ERROR_LOG_FMT(VIDEO,
                        "Failed to load custom pipeline action, asset id '{}' "
                        " has a key '{}' that has a value that is not a string!",
                        asset_id, key);
        }
        map[key] = fmt::format("{}/{}", path, value.to_str());
      }
      filesystem_library->SetAssetIDMapData(asset_id, std::move(map));
    }
  }
  ...
  return std::make_unique<CustomPipelineAction>(std::move(filesystem_library),
...
void CustomPipelineAction::OnTextureCreate(GraphicsModActionData::TextureCreate* create)
{
  ...
  create->additional_dependencies->emplace_back(pass.m_pixel_material.m_asset,
  ...
  create->additional_dependencies->emplace_back(pass.m_pixel_shader.m_asset,
                                                pass.m_pixel_shader.m_asset->GetLastLoadedTime());
    ...
    if (property.m_type == VideoCommon::MaterialProperty::Type::Type_TextureAsset)
    {
      if (property.m_value)
      {
        if (auto* value = std::get_if<std::string>(&*property.m_value))
        {
          ...
          game_assets.emplace_back(std::move(asset), loaded_time);
