...
GraphicsModRuntimeBackend::GraphicsModRuntimeBackend(const Config::GraphicsModGroup& config_data)
{
  for (auto& config_mod : config_data.GetMods())
  {
    ...
    for (const auto& tag : config_mod.m_mod.m_tags)
    {
      if (const auto iter = config_mod.m_mod.m_tag_name_to_action_indexes.find(tag.m_name);
          iter != config_mod.m_mod.m_tag_name_to_action_indexes.end())
      {
        for (const auto& index : iter->second)
        {
          tag_to_actions[tag.m_name].push_back(runtime_mod.m_actions[index].get());
        }
      }
    }
...
void GraphicsModRuntimeBackend::OnDraw(const DrawDataView& draw_data, VertexManagerBase* vertex_manager)
{
  if (m_mods.empty())
  {
