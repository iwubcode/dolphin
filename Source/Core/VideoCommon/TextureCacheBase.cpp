...
TCacheEntry* TextureCacheBase::LoadImpl(const TextureInfo& texture_info, bool force_reload)
{
  ...
  if (g_ActiveConfig.bGraphicMods)
  {
    ...
    texture.texture_type = GraphicsModSystem::Texture::TextureType::Normal;
    auto& system = Core::System::GetInstance();
  ...
  if (copy_to_vram)
  {
    ...
    if (entry)
    {
      ...
      if (g_ActiveConfig.bGraphicMods)
      {
        ...
        if (is_xfb_copy)
        {
          texture.texture_type = GraphicsModSystem::Texture::TextureType::XFB;
        }
        else
        {
          texture.texture_type = GraphicsModSystem::Texture::TextureType::EFB;
        }
        auto& mod_manager = system.GetGraphicsModManager();
        mod_manager.GetBackend().OnTextureCreate(texture);
      }
