...
void ActiveTargetsPanel::DrawImGui()
{
  ...
  if (m_filter_menu_active)
  {
    ...
    if (ImGui::BeginTable("FilterTable", 2))
    {
      ...
      ImGui::TableNextRow();
      ImGui::TableNextColumn();
      ImGui::Text("Skinning Type");
      ImGui::TableNextColumn();
      ImGui::BeginGroup();

      auto rb_skinning = [&](std::string_view text,
                             DrawCallFilterContext::SkinningFilter skinning_filter) {
        if (ImGui::RadioButton(text.data(),
                               m_drawcall_filter_context.skinning_filter == skinning_filter))
        {
          m_drawcall_filter_context.skinning_filter = skinning_filter;
        }
      };
  ...
  {
    GetEditorEvents().items_selected_event.Trigger(m_selected_nodes);
  }
  ...
  {
    if (Controls::ShowVertexGroupExtractWindow(m_state.m_vertex_group_dumper,
                                               m_last_vertex_group_dump_request))
    {
      m_open_vertex_group_dump_window = false;
      m_last_vertex_group_dump_request = {};
    }
  }
}
    ...
    const std::vector<GraphicsModEditor::RuntimeState::XFBData*>& xfbs)
{
  ...
  for (const auto xfb_data : xfbs)
  {
    for (const auto& draw_call_id_with_time : xfb_data->m_draw_call_ids)
    {
      ...
      const std::string id = fmt::to_string(std::to_underlying(draw_call_id));
      ...
      if (!ignore_target_context_menu)
      {
        ...
        if (ImGui::BeginPopup(id.data()))
        {
          ...
          if (ImGui::BeginMenu("Export"))
          {
            ...
            {
              m_state.m_editor_data.m_export_dolphin_material_draw_call = draw_call_id;
            }
          ...
          if (ImGui::BeginMenu("Create"))
          {
            ...
            {
              m_state.m_editor_data.m_create_material_draw_call = draw_call_id;
            }
            ImGui::EndMenu();
          }
          ImGui::EndPopup();
...
void ActiveTargetsPanel::TexturesPanel(
    const std::vector<GraphicsModEditor::RuntimeState::XFBData*>& xfbs)
{
  static constexpr ImGuiTreeNodeFlags base_flags = ImGuiTreeNodeFlags_OpenOnArrow |
                                                   ImGuiTreeNodeFlags_OpenOnDoubleClick |
  ...
  for (const auto xfb_data : xfbs)
  {
    for (const auto& texture_cache_id : xfb_data->m_texture_cache_ids)
    {
      ...
      if (!runtime_data.active)
        continue;

      ImGui::BeginGroup();

      const float column_width = ImGui::GetContentRegionAvail().x;
      const auto texture_view = runtime_data.texture;
      ...
      const float final_width = std::min(image_width, column_width / column_count);
      image_width = final_width;
      image_height = final_width / image_aspect_ratio;

      if (ImGui::ImageButton(texture_cache_id.c_str(), *runtime_data.texture.texture_data,
    ...
    const std::vector<GraphicsModEditor::RuntimeState::XFBData*>& xfbs)
{
  ...
  for (const auto xfb_data : xfbs)
  {
    for (const auto light_id : xfb_data->m_light_ids)
    {
      ...
      const std::string id = fmt::to_string(std::to_underlying(light_id));
