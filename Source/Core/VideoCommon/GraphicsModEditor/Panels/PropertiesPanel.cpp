...
void PropertiesPanel::DrawCallIDSelected(const GraphicsModSystem::DrawCallID& selected_object)
{
  ...
  if (ImGui::BeginTable("DrawCallBasicForm", 2))
  {
    ...
    ImGui::TableNextColumn();
    ImGui::Text("%llu", data.m_create_time.time_since_epoch().count());
  ...
  if (ImGui::CollapsingHeader("Textures", ImGuiTreeNodeFlags_DefaultOpen))
  {
    if (ImGui::BeginTable("DrawTexturesForm", 2))
    {
      for (const auto& texture : data.draw_data.textures)
      {
        ...
        {
          auto& sampler = data.draw_data.samplers[texture_view.unit];

          ImGui::TableNextRow();
          ImGui::TableNextColumn();

          ImGui::Text("Sampler (%i)", texture_view.unit);
          ...
          {
            ImGui::Text("<Texture %s last created/updated: %llu/%llu>", texture.hash_name.data(),
                        texture_info.m_create_time.time_since_epoch().count(),
