  ...
  if (ImGui::BeginTable("AsetBrowserContentTable", column_count))
  {
    ...
    for (const auto& entry : directory_entries)
    {
      if (std::filesystem::is_regular_file(entry))
      {
        ...
        else if (ext == ".rastershader")
        {
          ImGui::TableNextColumn();
          ImGui::BeginGroup();
          HandleAsset(entry, m_state.m_editor_data.m_name_to_texture["code"].get(),
                      "RasterShaderAsset");
          ImGui::EndGroup();
          columns_displayed++;
        }
    ...
    if (m_is_mesh_import_active)
    {
      if (Controls::ShowMeshImportWindow(m_mesh_import_filename, &m_mesh_import_import_materials,
                                         &m_mesh_import_process_gpu_skinning))
      {
        m_is_mesh_import_active = false;
        m_mesh_import_import_materials = false;
        m_mesh_import_process_gpu_skinning = false;
      }
    }
                                           ...
                                           ImGuiPopupFlags_NoOpenOverExistingPopup))
    {
      if (ImGui::BeginMenu("Create"))
      {
        ...
        {
          const std::string name = FindNewName("New Material", ".rastermaterial", m_current_path);
          const auto asset = GenerateMaterial(
              name, m_current_path, m_state.m_editor_data.m_name_to_template["raster_material"],
              m_state.m_user_data.m_asset_library.get());
          if (asset)
          {
            ...
            BeginRename(asset->m_asset_id, PathToString(asset->m_asset_path.stem()));
          }
        }
        if (ImGui::MenuItem("Shader"))
        {
          const auto metadata_name = FindNewName("New Shader", ".rastershader", m_current_path);
          const std::string pixel_name = FindNewName("New Shader.ps", ".glsl", m_current_path);
          const std::string vertex_name = FindNewName("New Shader.vs", ".glsl", m_current_path);

          const auto asset =
              GenerateShader(m_current_path, pixel_name, vertex_name, metadata_name,
                             m_state.m_editor_data.m_name_to_template["raster_shader_ps"],
                             m_state.m_editor_data.m_name_to_template["raster_shader_vs"],
                             m_state.m_editor_data.m_name_to_template["raster_shader_meta"],
                             m_state.m_user_data.m_asset_library.get());
          if (asset)
          {
            EditorEvents::ItemsSelectedEvent::Trigger(std::set<SelectableType>{asset});
        ...
        if (ImGui::MenuItem("Render Target"))
        {
          const std::string name =
              FindNewName("New Render Target", ".rendertarget", m_current_path);
          VideoCommon::RenderTargetData data;
          data.m_texture_format = AbstractTextureFormat::RGBA8;
          data.m_sampler = {};
          const auto asset =
              GenerateRenderTarget(m_current_path, data, m_state.m_user_data.m_asset_library.get());
          if (asset)
          {
            EditorEvents::ItemsSelectedEvent::Trigger(std::set<SelectableType>{asset});
            GraphicsModEditor::EditorEvents::ChangeOccurredEvent::Trigger();
