                  ...
                  std::span<const AssetDataType> asset_types_allowed)
{
  ...
  if (!asset)
  {
    if (ImGui::Button(fmt::format("None##{}", popup_name).c_str(), asset_button_size))
    {
  ...
  else
  {
    ...
    {
      if (ImGui::Button(PathToString(asset->m_asset_path.stem()).c_str(), asset_button_size))
      {
        if (!ImGui::IsPopupOpen(popup_name.data()))
        {
          asset_filter_text = "";
    ...
    if (ImGui::BeginTable("AssetBrowserPopupTable", column_count))
    {
      ...
      for (const auto& asset_from_library : state->m_user_data.m_asset_library->GetAllAssets())
      {
        ...
        else
        {
          ...
          if (ImGui::Button(fmt::format("{}##{}", name, asset_from_library.m_asset_id).c_str(),
                            asset_button_size))
          {
            *asset_id_chosen = asset_from_library.m_asset_id;
            changed = true;
