                              ...
                              VideoCommon::RasterShaderData* shader)
{
  ...
  if (ImGui::CollapsingHeader("Pixel Textures", ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    for (std::size_t i = 0; i < shader->m_pixel_samplers.size(); i++)
    {
      ImGui::PushID(static_cast<int>(i));
      auto& sampler = shader->m_pixel_samplers[i];
  ...
  if (ImGui::CollapsingHeader("Pixel Output Target", ImGuiTreeNodeFlags_DefaultOpen))
  {
    ImGui::PushID("Output Target");
    std::vector<VideoCommon::RasterShaderData::OutputTargetData*> output_targets_to_erase;
    for (std::size_t i = 0; i < shader->m_output_targets.size(); i++)
    {
      ImGui::PushID(static_cast<int>(i));
      auto& output_target = shader->m_output_targets[i];
      ImGui::InputText("Name", &output_target.name, ImGuiInputTextFlags_CharsNoBlank);

      ImGui::SameLine();
      if (ImGui::Button("X"))
      {
        output_targets_to_erase.push_back(&output_target);
      }
      ImGui::PopID();
    }

    for (const auto output_target : output_targets_to_erase)
    {
      std::erase(shader->m_output_targets, *output_target);
    }

    if (ImGui::Button("Add"))
    {
      VideoCommon::RasterShaderData::OutputTargetData output_target;
      output_target.name = "";
      shader->m_output_targets.push_back(std::move(output_target));
      GraphicsModEditor::EditorEvents::ChangeOccurredEvent::Trigger();
    }
