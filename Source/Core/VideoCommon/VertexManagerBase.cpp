...
bool VertexManagerBase::Initialize()
{
  ...
  auto& manager = system.GetGraphicsModManager();
  m_index_generator.Init(manager.IsEditorEnabled());
                                                       ...
                                                       u32 count, u32 stride, bool cullall)
{
  ...
  const bool supports_primitive_restart =
      g_ActiveConfig.backend_info.bSupportsPrimitiveRestart && !manager.IsEditorEnabled();
...
u32 VertexManagerBase::GetRemainingIndices(OpcodeDecoder::Primitive primitive) const
{
  ...
  const bool supports_primitive_restart =
      g_ActiveConfig.backend_info.bSupportsPrimitiveRestart && !mod_manager.IsEditorEnabled();

  if (primitive >= Primitive::GX_DRAW_LINES)
  {
    if (g_Config.UseVSForLinePointExpand() && !mod_manager.IsEditorEnabled())
    {
  ...
  if (!m_cull_all)
  {
    for (const u32 i : used_textures)
    {
      ...
      if (g_ActiveConfig.bGraphicMods)
      {
        ...
        {
          texture.texture_type = GraphicsModSystem::Texture::EFB;
        }
        else if (cache_entry->is_xfb_copy)
        {
          texture.texture_type = GraphicsModSystem::Texture::XFB;
        }
        else
        {
          texture.texture_type = GraphicsModSystem::Texture::Normal;
        }
        texture.unit = i;
...
void VertexManagerBase::OnConfigChange()
{
  ...
  m_index_generator.Init(mod_manager.IsEditorEnabled());
}
