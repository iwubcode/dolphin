...
{
  const auto config = ShaderConfig::CreateDefaultShader();
  s_default_shader = std::make_unique<VertexPixelShader>();
  s_default_shader->CreateOptions(config);
  s_default_shader->CreatePasses(config);
  s_default_shader->UpdateConfig(config);
}
...
void ShaderGroup::Apply(const ShaderApplyOptions& options)
{
  ...
  const auto update_default_shader = [&](const AbstractTexture* last_texture) {
    if (m_default_target_width != dest_rect_width || m_default_target_height != dest_rect_height ||
        m_default_target_layers != options.m_dest_fb->GetLayers() ||
        m_default_target_format != options.m_dest_fb->GetColorFormat())
    {
      const bool rebuild_pipelines =
          m_default_target_format != options.m_dest_fb->GetColorFormat() ||
          m_default_target_layers != options.m_dest_fb->GetLayers();
      m_default_target_width = dest_rect_width;
      m_default_target_height = dest_rect_height;
      m_default_target_layers = options.m_dest_fb->GetLayers();
      m_default_target_format = options.m_dest_fb->GetColorFormat();

      s_default_shader->CreateAllPassOutputTextures(dest_rect_width, dest_rect_height,
                                                    options.m_dest_fb->GetLayers(),
                                                    options.m_dest_fb->GetColorFormat());
      s_default_shader->UpdatePassInputs(last_texture);
      if (rebuild_pipelines)
      {
        s_default_shader->RebuildPipeline(m_default_target_format, m_default_target_layers);
      }
    }
  };
  if (m_shaders.empty() || m_skip)
  {
    const bool skip_final_copy =
        options.m_dest_fb->GetColorAttachment() != options.m_source_color_tex &&
        options.m_dest_fb->GetSamples() == 1;

    update_default_shader(options.m_source_color_tex);
    s_default_shader->Apply(skip_final_copy, options, options.m_source_color_tex, 1.0f);
    return;
  }
  ...
  if (!skip_final_copy)
  {
    update_default_shader(last_pass->m_output_texture.get());
    s_default_shader->Apply(true, options, last_pass->m_output_texture.get(), 1.0f);
  }
}
